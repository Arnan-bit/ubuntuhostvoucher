...
import { getBlogImage, getImageProps } from '@/lib/image-utils';
              ...
              return (
                <Card key={index} className="flex flex-col bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow duration-300">
                   <Link href={`/blog/${article.id}`} passHref>
                      <Image {...getImageProps(getBlogImage(article), article.title)} width={600} height={400} className="w-full h-48 object-cover" data-ai-hint={article.image_hint || article.imageHint || "blog post"} />
                  </Link>
