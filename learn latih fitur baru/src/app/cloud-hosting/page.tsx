...
export default function CloudHostingPage() {
  ...
  const cloudHostingDeals = allDeals.filter((d: any) => d.type === 'Cloud Hosting');

  const pageData = {
    Personal: cloudHostingDeals.filter((d: any) => d.tier === 'Personal'),
    Business: cloudHostingDeals.filter((d: any) => d.tier === 'Business'),
    Enterprise: cloudHostingDeals.filter((d: any) => d.tier === 'Enterprise'),
  };

  return (
    <ComparisonPage
      page="cloud-hosting"
      title={translations.en.cloudHosting}
      data={pageData}
      allDeals={allDeals}
      bannerConfig={siteSettings.pageBanners?.['cloud-hosting']}
