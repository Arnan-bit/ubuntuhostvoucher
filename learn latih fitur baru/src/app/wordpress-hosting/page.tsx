...
export default function WordpressHostingPage() {
  ...
  const wordpressHostingDeals = allDeals.filter(
    (d: any) => d.type === 'WordPress Hosting'
  );

  const pageData = {
    Personal: wordpressHostingDeals.filter((d: any) => d.tier === 'Personal'),
    Business: wordpressHostingDeals.filter((d: any) => d.tier === 'Business'),
    Enterprise: wordpressHostingDeals.filter((d: any) => d.tier === 'Enterprise'),
  };

  return (
    <ComparisonPage
      page="wordpress-hosting"
      title={translations.en.wordpressHosting}
      data={pageData}
      allDeals={allDeals}
      bannerConfig={siteSettings.pageBanners?.['wordpress-hosting']}
