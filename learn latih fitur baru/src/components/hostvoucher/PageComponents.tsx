        ...
        return (
             <div className="container mx-auto max-w-2xl p-4 pt-8 md:pt-16">
                <div className="text-center mb-12">
                     <Link href="/landing">
                        <Image src={siteAppearance?.specialistImageUrl || "https://placehold.co/128x128/f97316/ffffff/png?text=DH"} width={128} height={128} data-ai-hint="logo digital" alt="HostVoucher Profile" className="w-32 h-32 rounded-full mx-auto mb-4 border-4 border-orange-500 shadow-lg"/>
                    </Link>
                    <Link href="/landing">
...
const ProofOfPurchaseSection = ({ onProofSubmit, gamificationState }: any) => {
    const [proofForm, setProofForm] = useState({ fullName: '', whatsappNumber: '', provider: '', domain: '', purchaseDate: '', screenshot: null as File | null });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!proofForm.screenshot || !proofForm.fullName || !proofForm.whatsappNumber || !proofForm.provider || !proofForm.domain || !proofForm.purchaseDate) {
            toast({ title: "All fields are required", description: "Please fill out all fields and upload a screenshot.", variant: "destructive" });
            return;
        }
        setIsUploading(true);
        try {
            await onProofSubmit(proofForm);
            toast({ title: "Proof Submitted!", description: "Your submission is under review and 50,000,000 points have been added!" });
            setProofForm({ fullName: '', whatsappNumber: '', provider: '', domain: '', purchaseDate: '', screenshot: null });
        } catch (error: any) {
            ...
            <div className="p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                    ...
                    <div>
                            ...
                            <div className="space-y-1 text-center">
                                ...
                                <div className="flex text-sm text-gray-600 dark:text-gray-400">
                                    <label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                        ...
                                        <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={(e: any) => setProofForm({...proofForm, screenshot: e.target.files[0]})} accept="image/*" />
                                    </label>
                        ...
                        {proofForm.screenshot && <p className="text-sm text-green-500 mt-2">File selected: {(proofForm.screenshot as File).name}</p>}
                    </div>
...
export const RequestAndSubmitPage = ({ translations, allDeals, siteSettings, miningTasks, nftShowcase, hostvoucherTestimonials }: any) => {
    ...
    return (
        <section className="py-16"><div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            ...
            <div className="py-12">
                 <FAQSection page="request" language="en" translations={translations}/>
            </div>
