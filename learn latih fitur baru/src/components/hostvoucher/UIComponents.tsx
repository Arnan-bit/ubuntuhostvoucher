...
import { Share2, Sparkles, Link as LinkIcon, Twitter, Facebook, Instagram, MessageCircle, Youtube, Linkedin, Twitch, Github, Shell, Dribbble, Check } from 'lucide-react';
...
export const StarRating = ({ rating, numReviews, language = 'en', justify = 'center', onRatingChange }: any) => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        stars.push(
            <button key={i} onClick={() => onRatingChange && onRatingChange(i + 1)} aria-label={`Rate ${i + 1} stars`} className={onRatingChange ? 'cursor-pointer' : 'cursor-default'}>
                <Star type={type} />
            </button>
        );
    }
    ...
    return (
        <div className={`flex items-center mb-2 justify-${justify}`}>
            <div className="flex" aria-label={`Rating: ${numericRating} out of 5 stars`}>
                {stars}
            </div>
            {numReviews !== undefined && (
                <p className="ml-2 text-sm font-semibold text-gray-700 dark:text-gray-200">
                    {numericRating.toFixed(1)}/5 ({numReviews.toLocaleString()} {reviewText})
                </p>
            )}
        </div>
    );
...
export const ScrollerCard = ({ id, provider, name, price, link, rating, numReviews, image, language, currency, formatPrice }: any) => (
    <a href={link} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
        <div className="relative p-4 rounded-xl shadow-lg flex flex-col justify-between text-center transition-transform duration-300 h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 transform hover:scale-105">
            <Image src={image || "https://placehold.co/400x300.png"} alt={name} width={400} height={300} className="w-full h-32 object-cover rounded-md mb-4" data-ai-hint="deal offer" />
...
export const ServiceCard = ({ id, catalog_number, provider, provider_logo, name, price, original_price, discount, features, link, target_url, rating, num_reviews, language, currency, formatPrice, onAiSummary, isRecommended, button_color = 'blue', settings, animationStyle, seo_description }: any) => {
    const plan = { id, provider, name, price, original_price, discount, features, link: target_url || link, rating, num_reviews, type: 'Service' };
    ...
    // Ensure features is an array, parsing if it's a JSON string
    let parsedFeatures = [];
    if (typeof features === 'string') {
        try {
            parsedFeatures = JSON.parse(features);
        } catch (e) {
            // If parsing fails, treat it as a single feature or split by newline
    ...
    return (
      <div id={`deal-${id}`} className={`relative p-6 rounded-xl flex flex-col justify-between text-center transform hover:scale-105 transition-all duration-300 h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 ${recommendedClass} ${cardLightColorClasses[button_color]} ${animationClasses[animationStyle]}`}>
        ...
        <div className="flex-shrink-0 mb-4">
          ...
          <div className="mt-2">
            {provider_logo ? <Image src={provider_logo} alt={provider} width={120} height={40} className="h-10 mx-auto object-contain mb-2" /> : <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-1">{provider}</p>}
            <h3 className="text-2xl font-bold mb-2 text-gray-900 dark:text-white">{name}</h3>
             <p className="text-sm text-gray-500 dark:text-gray-400 mb-2 h-10 overflow-hidden">{seo_description}</p>
            {original_price && parseFloat(original_price as string) > parseFloat(price) && (
              <p className="text-red-500 line-through text-lg font-medium">
                {formatPrice(original_price, currency)}
              </p>
            )}
            {(rating !== undefined && num_reviews !== undefined) && (
                <StarRating rating={rating} numReviews={num_reviews} language={language} />
            )}
            <p className="text-4xl font-extrabold text-blue-600 dark:text-blue-400 mb-4">{formatPrice(price, currency)}</p>
            {discount && (
        ...
        <div className="flex-grow flex flex-col justify-end">
          {parsedFeatures && Array.isArray(parsedFeatures) && parsedFeatures.length > 0 && (
            <ul className="text-gray-600 dark:text-gray-300 mb-6 space-y-2 text-left w-full border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          <div className="mt-auto">
              <a href={target_url || link} onClick={handleClick} target="_blank" rel="noopener noreferrer" className={`block w-full bg-gradient-to-r text-white py-3 px-6 rounded-full font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 ${buttonColorClasses[button_color]}`}>
...
export const UniversalSearchResultCard = ({ item, language, currency, formatPrice, onCopy, animationStyle }: any) => {
    const { id, name, title, provider, seoDescription, price, originalPrice, discount, link, targetUrl, image, providerLogo, rating, numReviews, type, code, catalogNumber } = item;
    const placeholderImage = providerLogo || `https://placehold.co/100x100.png?text=${(provider || 'Deal').substring(0, 6)}`;
    ...
    return (
        <a href={displayLink} target="_blank" rel="noopener noreferrer" className="w-full text-left relative flex items-start p-4 rounded-lg transition-all duration-300 transform hover:scale-[1.02] shadow-sm hover:shadow-xl bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
            ...
            <Image src={displayImage} alt={displayName} width={100} height={100} className="w-20 h-20 md:w-24 md:h-24 rounded-md mr-4 object-contain bg-white p-1" onError={(e: any) => { e.target.onerror = null; e.target.src=placeholderImage; }} />
...
export const ChatBot = ({ translations }: any) => {
    ...
    useEffect(() => {
        if(isOpen && messages.length === 0) {
             setMessages([{ sender: 'ai', text: translations[language].chatbotWelcome }]);
        }
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            setMessages([...newMessages, { sender: 'ai', text: "I'm sorry, my AI brain is still under construction. Please check back later!" }]);
    ...
    return (
        <>
            <button 
                ...
                className="fixed bottom-5 right-5 w-16 h-16 bg-orange-500 text-white rounded-full shadow-2xl z-[60] flex items-center justify-center hover:bg-orange-600 transition-transform transform hover:scale-110"
            ...
            {isOpen && (
                <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 20 }}
                    className="fixed bottom-24 right-5 z-[60] transition-all duration-300 ease-in-out">
                     <div className="w-80 h-[28rem] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col border border-gray-200 dark:border-gray-700">
                        <header className="bg-orange-500 dark:bg-orange-600 text-white p-3 rounded-t-2xl flex justify-between items-center">
                            <h3 className="font-bold text-lg">{translations[language].chatbotTitle || "HostVoucher AI Assistant"}</h3>
                        ...
                        <form onSubmit={handleSendMessage} className="p-3 border-t border-gray-200 dark:border-gray-700 flex items-center gap-2">
                            ...
                            />
                            <button type="submit" className="bg-orange-500 text-white rounded-full p-3 hover:bg-orange-600 transition-colors disabled:opacity-50" disabled={!userInput.trim() || isAiLoading}>
                                <SendIcon />
                            </button>
                        </form>
                    </div>
...
export const FloatingPromotionalPopup = ({ siteAppearance }: any) => {
    ...
    useEffect(() => {
        ...
        if (promoUrl && !hasBeenClosed) {
            const timer = setTimeout(() => setIsVisible(true), 5000); // Delay of 5 seconds
    ...
    const PromoContent = () => (
        <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-24 right-5 z-[59] w-36 h-36 rounded-2xl shadow-2xl overflow-hidden cursor-pointer"
        ...
        >
            ...
            <button
                onClick={handleClose}
                className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1.5 hover:bg-black/70 transition-colors"
...
export const RotatingBanner = ({ bannerConfig, showMarketingOverlay, onButtonClick }: any) => {
    ...
    if (!slides || slides.length === 0 || !slides.some((s:any) => s.imageUrl)) {
        return null;
    }
    ...
    return (
        <div className="relative w-full max-w-7xl mx-auto my-12 overflow-hidden rounded-lg shadow-2xl" style={{ aspectRatio: '16 / 9' }}>
            <AnimatePresence initial={false}>
                ...
                >
                    <Image
                        src={slides[currentIndex].imageUrl || 'https://placehold.co/1280x720.png'}
        ...
        <section className="py-16 bg-gray-50 dark:bg-gray-900 overflow-hidden">
            <div className="text-center mb-12">
                <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white">
                    Trusted by 8+ million website owners worldwide
                </h2>
            </div>
